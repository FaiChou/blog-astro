...
问题二，如何知道第 k 个点代表哪个频率？答案: `f(k) = k x (fs/N)`, fs 是采样率，总共有 N 个采样点。所以通过公式可以知道，对于同一首音乐，截取10秒钟和截取1分钟获取的音乐数据，虽然1分钟的时长长一些，采样点多一些，但**最高的频率**是和10秒的一样，因为采样率是相同的。其值为 fs/2（采样率的一半）。例如，如果采样率 fs = 44.1 kHz（CD音质），那么无论你分析1秒还是1小时的音频，FFT能看到的最高频率都是 44.1kHz / 2 = 22.05 kHz。这个值是由硬件（采样率）决定的，与音频内容时长无关。但采样点多可以有更丰富的频谱数据。

对于音频的采样数据不能直接处理，因为数据可能不好看，会有泄漏效应和边缘效应。所以需要使用窗口函数用来对短时信号进行加权。常用的窗口函数有矩形窗，汉宁窗，汉明窗和布莱克曼窗。拿汉宁窗(`vDSP_hann_window`)来举例，它在两端逐渐降到0，中间部分的权重较高，可以有效减少泄漏效应和边缘效应。在 swift 中可以这样操作:
