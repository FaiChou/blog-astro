...
如果不进行 Gamma 校正，图像会看起来过暗。所以在存储和传输图像时，需要先进行 Gamma 编码（预校正），显示时再由显示器的 Gamma 特性自然解码。

白点（White Point）定义了"白色"的标准。不同的光源有不同的色温，D65 是最常用的标准白点，对应色温约 6500K 的日光。sRGB、Display P3 等色彩空间都使用 D65 作为白点。其他常见的白点还有 D50（5000K，印刷行业）和 D55（5500K）。
