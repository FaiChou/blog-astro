...
- S 型（Short）：峰值灵敏度约 420nm，对应蓝色
- M 型（Medium）：峰值灵敏度约 530nm，对应绿色
- L 型（Long）：峰值灵敏度约 560nm，对应红色

这里的"峰值"容易引起误解。实际上每种视锥细胞的响应范围都很宽，三者之间有大量重叠。S 型细胞虽然峰值在 420nm，但它对 380nm-540nm 的光都有响应；M 和 L 型的响应范围几乎完全重叠，都覆盖 400nm-700nm，只是峰值位置略有不同。正因为这种重叠，大脑才能通过比较三种细胞的相对响应强度来推断颜色。如果三种细胞各管各的波段互不重叠，颜色感知反而会很粗糙。
...
iOS 的视频解码使用 VideoToolbox，解码后输出的是 `CVPixelBuffer`，这是一个包含 YCbCr 数据的像素缓冲区。对于 4:2:0 格式，它通常是双平面的（Bi-Planar）：
