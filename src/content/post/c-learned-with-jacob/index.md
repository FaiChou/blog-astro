...
description: "跟着Jacob Sorber 老师学习 C 语言"
...
1. 取 p 的当前值（即当前指针指向的地址）。
2.	对该地址进行解引用（*），并将 c 赋值给该地址的内容。
3.	将 p 的值自增，指向下一个地址。
